...
proc initHookKdl*(v: var MyObj2) =
  v.id = 100

proc postHookKdl*(v: var MyObj2) =
  inc v.id
...
suite "Decoder":
  ...
  test "Array":
    ...
    check parseKdl("node {a 1; b 2}; node {a 3; b 3}").decodeKdl(array[2, Foo]) ==
      @[Foo(a: 1, b: 2), Foo(a: 3, b: 3)]
  ...
  test "Object & Tuple":
    ...
    check parseKdl("person age=20 {name \"Rika\"}").decodeKdl(
      tuple[age: int, name: string], "person"
    ) == (age: 20, name: "Rika")
    check parseKdl(
      "name \"Mindustry\"; version \"126.2\"; author \"Anuken\"; license \"GNU General Public License v3.0\""
    )
    .decodeKdl(Game) ==
      Game(
        name: "Mindustry",
  ...
  test "Object Variant":
    ...
    .decodeKdl(seq[MyObj]) ==
      @[
        MyObj(kind: moString, stringV: "Hello"),
        MyObj(kind: moInt, intV: 12),
        MyObj(kind: moString, stringV: "Beef", kind2: moInt, intV2: 0xbeef),
      ]
  ...
  test "Enum":
    ...
    when defined(kdlDecoderAllowHoleyEnums):
      check parseKdl("dir 2 3").decodeKdl(seq[HoleyDir], "dir") == @[
        HoleyDir(2), hSouth
      ]
    else:
      expect KdlError:
  ...
  test "HashSet and OrderedSet":
    check parseKdl("- \"a\" \"b\" \"c\"")[0].decodeKdl(HashSet[string]) ==
      ["a", "b", "c"].toHashSet
...
table mlicb="F"; table2 hakumuse=500; table3 a="b" c="d"
  ...
  test "custom decodeKdl":
    ...
    check parseKdl("date 2022 \"October\" 15 12 04 00").decodeKdl(DateTime, "date") ==
      dateTime(2022, mOct, 15, 12, 04)

    check parseKdl("author birthday=\"2000-10-15\" name=\"Nobody\"")[0]["birthday"].decodeKdl(
      DateTime
    ) == dateTime(2000, mOct, 15)
  ...
  test "decodeRenameKdl":
    ...
    .decodeKdl(MyObj) ==
      MyObj(kind: moString, stringV: "hello", kind2: moInt, intV2: 0xbeef)
    ...
    .decodeKdl(MyObj, "node") ==
      MyObj(kind: moString, stringV: "bye", kind2: moInt, intV2: 0xdead)
...
suite "Encoder":
  test "Crate":
    ...
    check encodeDecodesDoc Package(
      name: "kdl",
      version: "0.0.0",
      authors: @["Kat Marchán <kzm@zkat.tech>"].some,
  ...
  test "Tables":
    check encodeDecodesDoc {"key": "value".initKVal, "alive": true.initKVal}.toTable
    check encodeDecodesDoc {
      "age": 10.initKVal, "name": "Phil".initKVal, "other-name": "Isofruit".initKVal
    }.toTable

    check encodeDecodesDoc {"key": "value".initKVal, "alive": true.initKVal}.toOrderedTable
    check encodeDecodesDoc {
      "age": 10.initKVal, "name": "Phil".initKVal, "other-name": "Isofruit".initKVal
    }.toOrderedTable
